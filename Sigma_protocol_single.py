...
_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_1 = Integer(1)
from setup import *
...
class Commitment():
    def __init__(self,pp,sk,t,R,seed=None,rd0=None):
        ...
        self.a=_sage_const_0 
        self.rd=[]
        self.path=[]
        if rd0==None:
            self.rd0=ZZ.random_element(_sage_const_2 **(pp.lamda))
        else:
            self.rd0=rd0
        self.pp=pp
        self.sk=sk
    ...
    def com(self):
        ...
        for i in range(n):
            if i==t:
                ...
                j3=E3.j_invariant()
                C1=self.pp.C(j3.to_bytes(),self.rd[i].to_bytes(pp.lamda/_sage_const_8 ))
                ...
                xK=self.vect_K_phi_prim[_sage_const_1 ]
                L1=xK.parent()
                xT=self.vect_K_psi[_sage_const_1 ]
                L2=xT.parent()
                ...
                #hx=(self.rd0).str(base=16)
                #bytes_rd=bytes.fromhex(hx)
                C0=pp.C(self.j2.to_bytes(),self.rd0.to_bytes(pp.lamda/_sage_const_8 ))  
            else:
                E1=self.R[i]
                E3=CGL(E1,self.a,d2,k2,return_kernel=False)
                j3=E3.j_invariant()
                
                C1=pp.C(j3.to_bytes(),self.rd[i].to_bytes(pp.lamda/_sage_const_8 ))
                vect_com_R.append(C1)
        root,self.path=self.pp.hidden_merkle_tree(vect_com_R,vect_com_R[self.t])
...
def response(pp,sk,R,t,com,ch,with_rd=False):
    assert ch in [_sage_const_2 ,_sage_const_0 ,_sage_const_1 ]
    if ch==_sage_const_2 :
        if with_rd:
...
def verivication(pp,R,com,ch,resp):
    ...
    if ch==_sage_const_2 :
        [vect_K_psi,rd0]=resp
        xT0=vect_K_psi[_sage_const_0 ]
        xT1=vect_K_psi[_sage_const_1 ]
        L0=xT0.parent()
        L1=xT1.parent()
        ...
        if pp.C(j2.to_bytes(),rd0.to_bytes(pp.lamda/_sage_const_8 ))!=C0:
    ...
    if ch==_sage_const_1 :
        ...
        d=pp.d2()
        for i in range(n):
            E3=CGL(R[i],a,d,_sage_const_2 ,return_kernel=False)
            j3=E3.j_invariant()
            C2=pp.C(j3.to_bytes(),rd[i].to_bytes(pp.lamda/_sage_const_8 ))
            vect_com.append(C2)
    ...
    if ch==_sage_const_0 :
        ...
        if pp.C(j2.to_bytes(),rd0.to_bytes(pp.lamda/_sage_const_8 ))!=C0:
            ...
            return False
        xK0=vect_K_phi_prim[_sage_const_0 ]
        xK1=vect_K_phi_prim[_sage_const_1 ]
        L0=xK0.parent()
        E3=L0.curve()
        j3=E3.j_invariant()
        Ct=pp.C(j3.to_bytes(),rd.to_bytes(pp.lamda/_sage_const_8 ))
