...
_sage_const_0 = Integer(0); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_7 = Integer(7); _sage_const_6 = Integer(6); _sage_const_4 = Integer(4)
import inspect
import os
import sys
import sage.all
...
class SetUp():
    def __init__(self,lamda,linkeable=False):
        assert lamda>_sage_const_0 
        self.l1=_sage_const_3 
        self.l2=_sage_const_5 
        self.lamda=lamda
        self.k1=_sage_const_2 
        self.k2=_sage_const_2 
        self.N=_sage_const_1 
        self.e1=ceil(self.lamda/log(self.l1,_sage_const_2 ))
        self.e2=ceil(self.lamda/log(self.l2,_sage_const_2 ))
        if linkeable:
            self.N=_sage_const_7 **(ceil(lamda/log(_sage_const_7 ,_sage_const_2 )))
    ...
    def initial_curve(self):
        ...
        Fp=GF(p)
        R=Fp["x"]
        self.Fq=GF(p**_sage_const_2 ,'i',modulus=x**_sage_const_2 +_sage_const_1 )
        return EllipticCurve(self.Fq,[_sage_const_0 ,_sage_const_6 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ])
    ...
    def H(self,data):  #The hash function
        ...
        hashed=h.digest(self.lamda/_sage_const_4 )
    ...
    def hidden_merkle_tree(self,leaves,sibling_leaf=None):
        ...
        if sibling_leaf!= None:
            ...
            if j%_sage_const_2 ==_sage_const_0 :
                path.append([tree[j+_sage_const_1 ],_sage_const_0 ])
            else:
                path.append([tree[j-_sage_const_1 ],_sage_const_1 ])
        # Build the tree
        while len(tree) > _sage_const_1 :
            if len(tree) % _sage_const_2  == _sage_const_1 :
                tree.append(tree[-_sage_const_1 ])  # Duplicate the last element if odd
            new_tree = []
            for i in range(_sage_const_0 , len(tree), _sage_const_2 ):
                h=self.H(bytes(tree[i]) + bytes(tree[i + _sage_const_1 ]))
                new_tree.append(h)
                if leaf!= None:
                    if j in [i,i+_sage_const_1 ] and len(tree)>_sage_const_2 :
                        leaf=h
            tree = new_tree
            if sibling_leaf!= None:
                if len(tree)>=_sage_const_2 :
                    j=tree.index(leaf)
                    if j%_sage_const_2 ==_sage_const_0 :
                        path.append([tree[j+_sage_const_1 ],_sage_const_0 ])
                    else:
        ...
        if sibling_leaf!= None:
            return tree[_sage_const_0 ],path  # Return the root of the Merkle tree
        else:
            return tree[_sage_const_0 ]
    ...
    def reconstructRoot(self,leaf,path):
        ...
        for i in range(len(path)):
            node=path[i]
            if node[_sage_const_1 ]==_sage_const_0 :
                root=self.H(bytes(root)+bytes(node[_sage_const_0 ]))
            else:
                root=self.H(bytes(node[_sage_const_0 ])+bytes(root))
        return root
